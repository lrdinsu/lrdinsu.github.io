...
- [Go Wiki: Mutex or Channel?](https://go.dev/wiki/MutexOrChannel) — Official Go guidance on when to use mutexes versus channels. Workron’s mutex-protected map follows this recommendation for shared state protection.
- [How we designed Dropbox ATF: an async task framework](https://dropbox.tech/infrastructure/asynchronous-task-scheduling-at-dropbox) — Dropbox’s engineering write-up on an asynchronous task framework. It was a useful reference for callback-based execution, at-least-once delivery, and heartbeat-style worker monitoring.
- [Sidekiq Wiki: Reliability](https://github.com/sidekiq/sidekiq/wiki/Reliability) — Describes how Sidekiq handles orphaned jobs and worker recovery using heartbeats and Redis. This was a helpful reference when designing stale-job detection in Workron.
- [Design a Distributed Job Scheduler](https://blog.algomaster.io/p/design-a-distributed-job-scheduler) — A system design walkthrough covering coordinator patterns, worker failure handling, and scheduling concerns at scale.
- [Apache Airflow Architecture Overview](https://airflow.apache.org/docs/apache-airflow/stable/core-concepts/overview.html) — Overview of Airflow’s scheduler, worker, and DAG-based orchestration model. It serves as a useful reference point for workflow execution concepts.
